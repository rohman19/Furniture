...
class Administrator extends CI_Controller {
	...
	public function index()
	{
		...
		$data['trans'] = $this->admin->count_where('t_order', ['status_byr' => 'proses']);
		$data['last'] = $this->admin->last('t_order o JOIN t_users u ON(o.id_user = u.id_user)', 3, 'id_order' );

		$this->template->admin('admin/home', $data);
	}
function laporan_print_item(){
	$data['t_items'] = $this->admin->get_data('t_items')->result();
	$this->load->view('admin/laporan_print_item', $data);
}
function cetak_laporan_kategori(){
		$data['t_kategori'] = $this->admin->get_data('t_kategori')->result();
		
		$this->load->view('admin/laporan_kategori', $data);
		
	}

	function cetak_laporan_item(){
		$data['t_items'] = $this->admin->get_data('t_items')->result();
		
		$this->load->view('admin/laporan_item', $data);
		
	}
	function cetak_laporan_order(){

		$data['t_order'] = $this->admin->get_data('t_order o JOIN t_users u ON(o.id_user = u.id_user)', 3, 'id_order' )->result();
		
		$this->load->view('admin/laporan_order', $data);
		
	}
		function cetak_laporan_user(){

		$data['t_users'] = $this->admin->get_data('t_users')->result();
