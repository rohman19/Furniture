	...
	public function registrasi()
	{
		if($this->input->post('submit', TRUE) == 'Submit')
		{
			...
			if ($this->form_validation->run() == TRUE)
			{
				...
				{
				echo '<script type="text/javascript">alert("Daftar Berhasil, Silahkan Login");</script>';
				} else {
		...
		if ($this->session->userdata('user_loginr') == TRUE)
      {
         redirect('home');
      }
		...
		if ($this->input->post('submit') == 'Submit')
      {
         ...
         if ($cek->num_rows() > 0) {
            ...
            if (password_verify($pass, $data->password))
            {
               ...
               echo '<script type="text/javascript">alert("Password di tolak");</script>';
