		...
		if ($this->input->post('submit', TRUE) == 'Submit')
      {
         ...
         if ($this->form_validation->run() == TRUE)
         {
            ...
            if ($get->num_rows() > 0)
            {
               ...
               $config['useragent'] = 'CV Gemilang Lestari Jaya';
               $config['protocol'] = 'smtp';
               $config['mailtype'] = 'html';
               $config['smtp_host'] = 'ssl://smtp.gmail.com';
               $config['smtp_port'] = '465';
               $config['smtp_timeout'] = '5';
               $config['smtp_user'] = 'example@gmail.com'; //isi dengan email gmail
               $config['smtp_pass'] = 'example'; //isi dengan password
               $config['crlf'] = "\r\n";
               $config['newline'] = "\r\n";
               ...
               $this->email->from('example@gmail.com', "CV Gemilang Lestari Jaya");
               $this->email->to($user->email);
               $this->email->subject('Pembayaran');
               $this->email->message(
                  'Terima Kasih telah melakukan pemesanan di toko kami, selanjutnya silahkan anda mentransfer uang senilai <b>Rp. '.number_format($total, 0, ',', '.').',-</b> ke no. rekening <b>90213123</b> paling lambat '.$bts.' agar pesanan anda bisa kami proses. Detail pembayaran sebagai berikut :<br/><br/>
   ...
   public function city()
   {
      ...
      curl_setopt_array($curl, array(
        ...
        CURLOPT_HTTPHEADER => array(
          "key: 77789b66a34e71be1af8de0b21a9b2ec"
        ),
      ));
